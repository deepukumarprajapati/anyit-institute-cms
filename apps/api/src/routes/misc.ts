...
import { instituteFilter, parsePagination } from '../utils/query';
...
dashboardRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [
      ...
      Student.countDocuments({ ...filter, status: 'active', deletedAt: null }),
      Staff.countDocuments({ ...filter, status: 'active', deletedAt: null }),
      FeeInvoice.find({
        ...filter,
        ...sessionFeeFilter,
      ...
      FeeInvoice.find({
        ...
        ...sessionFeeFilter,
        deletedAt: null,
      }).select('status totalAmount paidAmount studentId sessionId'),
      ...
      Campus.find({ ...filter, deletedAt: null }).select('name code'),
    ...
    const staffDocs = staffIds.length
      ? await Staff.find({ _id: { $in: staffIds }, ...filter })
          ...
          .lean()
      : [];
    const staffInfo = new Map(staffDocs.map((s) => [String(s._id), s]));
